...
import { FaEnvelope, FaLock, FaSignInAlt, FaUserMd, FaEye, FaEyeSlash, FaStethoscope } from 'react-icons/fa';
...
const DoctorLogin = ({ embedded = false, setLoginInProgress, redirectUrl }) => {
  ...
  useEffect(() => {
    ...
    const rememberMe = localStorage.getItem('rememberMeDoctor') === 'true';
      ...
      if (result && result.success) {
        ...
        try {
          ...
          if (tokenParts.length === 3) {
            ...
            console.log('[DoctorLogin] Token payload:', tokenPayload);
            ...
            if (userId) {
              console.log('[DoctorLogin] Found userId in token:', userId);
              localStorage.setItem('userId', userId);
              localStorage.setItem('doctorId', userId);
            } else {
              console.warn('[DoctorLogin] Could not find userId in token');
              ...
              for (const key in tokenPayload) {
                if (key.toLowerCase().includes('id') || key.toLowerCase().includes('user')) {
                  console.log(`[DoctorLogin] Potential userId found in claim '${key}':`, tokenPayload[key]);
                  localStorage.setItem('userId', tokenPayload[key]);
                  localStorage.setItem('doctorId', tokenPayload[key]);
                  break;
                }
              }
            }
          ...
          console.error('[DoctorLogin] Error extracting userId from token:', tokenError);
        }
        ...
        <div className="w-full max-w-md">
          ...
          {!embedded && (
            <div className="mb-8 text-center">
              <div className="h-20 w-20 rounded-full bg-gradient-to-r from-indigo-500 to-blue-600 flex items-center justify-center mb-4 mx-auto shadow-lg">
                ...
                <div className="mt-6">
                  <div className="text-center">
                    <p className="text-sm font-medium text-gray-700">
                      ...
                      <Link to="/doctor/register" className="text-blue-600 hover:text-blue-500">
                        Apply here
                      </Link>
                    </p>
                  </div>
